...
#'
#' Another way to modify the modelling results is to change the 'method'. In this version, 'method' has four possible values: "slope.clustering", "max.square", "min.slope" and "min.similarity", which indicate the different four different modelling methods:
...
module.modeling<-function(res.module, keep.gene.num=NULL, method=c("slope.clustering", "max.square", "min.slope", "min.similarity")[1], cores=1, overlap=NULL, para=NULL){
	...
	mdl=mclapply(mods, function(mod){
		...
		sc=sc[sc > overlap];
		patients=names(sc);
		return(list(genes=used.genes, patients=patients, sc=sc, model.method=method))
